...
//       // Store the available slots for the day
...
//     try {
//       const date = docSlot[slotIndex][0]?.datetime; // Add optional chaining here
...
//                     docSlot.map(
//                       (item, index) =>
//                         item.length > 0 && ( // Ensure item has elements
//                           <div
...
//                   {docSlot.length > 0 &&
//                     docSlot[slotIndex]?.map(
//                       (
//                         item,
//                         index // Optional chaining to prevent errors
//                       ) => (
//                         <p
...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    ...
    setLoading(false); // Set loading to false after fetching slots
  };
  ...
  useEffect(() => {
    if (docInfo) {
      ...
      setLoading(false); // Set loading to false if doctor info is not found
    }
  }, [docInfo]);
